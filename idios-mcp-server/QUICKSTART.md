...
You want the privacy coin built on MimbleWimble called Beam privacy, from **beam.mw**. There is a completely different project, Beam Network, a gaming token on Avalanche, which has nothing to do with Beam Privacy. Everything below is the MimbleWimble Beam.
...
For testing, all you need is a little BEAM. The standard route is a centralised exchange: buy BEAM on MEXC, Gate, or CoinEx using USDT, BTC or ETH and withdraw it to your wallet. If you would rather skip exchange accounts, [buybeam.my](https://buybeam.my/) is a community run swap that takes ETH straight to BEAM and sends it to your Beam wallet address in seconds. Two limits: it has a minimum of $1 of BEAM, smaller amounts send nothing to your wallet, and it handles roughly $100 per swap at most (for larger buys use an exchange). For the full picture, supported exchanges, mining, and the wider Beam ecosystem, see [beam.mw](https://www.beam.mw/).
